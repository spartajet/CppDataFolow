...
#ifndef CPPDATAFOLOW_QUEUE_THREAD_POOL_H
#define CPPDATAFOLOW_QUEUE_THREAD_POOL_H
...
namespace cdf {
    ...
    private:
        ...
        atomic_int thread_cursor{0}; //当前线程的游标
        std::thread *manage_thread = nullptr;
        int size = 4;
    public:
        inline queue_thread_pool(short size = 4) {
            this->size = size;
            addThread(size);
            manage_thread = new std::thread([this] {
                while(_run){
//                    if ()
                }
            });
...
#endif //CPPDATAFOLOW_QUEUE_THREAD_POOL_H
